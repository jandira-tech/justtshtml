...
type ModeHandlerResult = ReprocessAction | null;
type ModeHandler = (self: any, token: Tag) => ModeHandlerResult;
...
function modeInitial(self: any, token: any): ModeHandlerResult {
...
function modeBeforeHtml(self: any, token: any): ModeHandlerResult {
...
function modeBeforeHead(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    let data = token.data || "";
...
function modeInHead(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
...
function modeInHeadNoscript(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    const data = token.data || "";
  ...
  if (token instanceof Tag) {
    if (token.kind === Tag.START) {
      ...
      if (["basefont", "bgsound", "link", "meta", "noframes", "style"].includes(token.name))
...
function modeAfterHead(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    let data = token.data || "";
    if (data.includes("\x00")) {
...
function modeText(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    self._append_text(token.data);
    return null;
  }
...
function handleCharactersInBody(self: any, token: CharacterToken): ModeHandlerResult {
  let data = token.data || "";
  if (data.includes("\x00")) {
...
function handleCommentInBody(self: any, token: CommentToken): ModeHandlerResult {
  self._append_comment(token.data);
  return null;
}

function handleBodyStartHtml(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartBody(self: any, token: Tag): ModeHandlerResult {
  if (self.template_modes.length) {
    self._parse_error("unexpected-start-tag", token.name);
    return null;
  }
...
function handleBodyStartHead(self: any, token: Tag): ModeHandlerResult {
  self._parse_error("unexpected-start-tag", token.name);
  return null;
}

function handleBodyStartInHead(self: any, token: Tag): ModeHandlerResult {
  return modeInHead(self, token);
}

function handleBodyStartBlockWithP(self: any, token: Tag): ModeHandlerResult {
  self._close_p_element();
  self._insert_element(token, { push: true });
  return null;
}

function handleBodyStartHeading(self: any, token: Tag): ModeHandlerResult {
  self._close_p_element();
  if (
...
function handleBodyStartPreListing(self: any, token: Tag): ModeHandlerResult {
  self._close_p_element();
  self._insert_element(token, { push: true });
  self.ignore_lf = true;
  self.frameset_ok = false;
...
function handleBodyStartForm(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartButton(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartParagraph(self: any, token: Tag): ModeHandlerResult {
  self._close_p_element();
  self._insert_element(token, { push: true });
  return null;
}

function handleBodyStartMath(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
...
function handleBodyStartSvg(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
...
function handleBodyStartLi(self: any, token: Tag): ModeHandlerResult {
  self.frameset_ok = false;
  self._close_p_element();
...
function handleBodyStartDdDt(self: any, token: Tag): ModeHandlerResult {
  self.frameset_ok = false;
  self._close_p_element();
...
function handleBodyStartA(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartFormatting(self: any, token: Tag): ModeHandlerResult {
  const name = token.name;
...
function handleBodyStartAppletLike(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
  self._insert_element(token, { push: true });
  self._push_formatting_marker();
  self.frameset_ok = false;
...
function handleBodyStartBr(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
  self._insert_element(token, { push: false });
  self.frameset_ok = false;
  return null;
...
function handleBodyStartHr(self: any, token: Tag): ModeHandlerResult {
  self._close_p_element();
  self._insert_element(token, { push: false });
  self.frameset_ok = false;
  return null;
...
function handleBodyStartFrameset(self: any, token: Tag): ModeHandlerResult {
  if (!self.frameset_ok) {
...
function handleBodyStartStructureIgnored(self: any, token: Tag): ModeHandlerResult {
  self._parse_error("unexpected-start-tag-ignored", token.name);
  return null;
}

function handleBodyStartColOrFrame(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartImage(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartVoidWithFormatting(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
  self._insert_element(token, { push: false });
  self.frameset_ok = false;
  return null;
...
function handleBodyStartSimpleVoid(self: any, token: Tag): ModeHandlerResult {
  self._insert_element(token, { push: false });
  return null;
}
...
function handleBodyStartTable(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyStartPlaintextXmp(self: any, token: Tag): ModeHandlerResult {
  self._close_p_element();
  self._insert_element(token, { push: true });
  self.frameset_ok = false;
...
function handleBodyStartTextarea(self: any, token: Tag): ModeHandlerResult {
  self._insert_element(token, { push: true });
  self.ignore_lf = true;
  self.frameset_ok = false;
  return null;
...
function handleBodyStartSelect(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
  self._insert_element(token, { push: true });
  self.frameset_ok = false;
  self._reset_insertion_mode();
...
function handleBodyStartOption(self: any, token: Tag): ModeHandlerResult {
  if (
    self.open_elements.length &&
...
function handleBodyStartOptgroup(self: any, token: Tag): ModeHandlerResult {
  if (
    self.open_elements.length &&
    self.open_elements[self.open_elements.length - 1].name === "option"
  ) {
...
function handleBodyStartRpRt(self: any, token: Tag): ModeHandlerResult {
  self._generate_implied_end_tags("rtc");
  self._insert_element(token, { push: true });
  return null;
}

function handleBodyStartRbRtc(self: any, token: Tag): ModeHandlerResult {
  if (
    self.open_elements.length &&
...
function handleBodyStartTableParseError(self: any, token: Tag): ModeHandlerResult {
  self._parse_error("unexpected-start-tag", token.name);
  return null;
}

function handleBodyStartDefault(self: any, token: Tag): ModeHandlerResult {
  self._reconstruct_active_formatting_elements();
  self._insert_element(token, { push: true });
  if (token.selfClosing)
    self._parse_error("non-void-html-element-start-tag-with-trailing-solidus", token.name);
  self.frameset_ok = false;
  return null;
}

const BODY_START_HANDLERS: Record<string, ModeHandler> = {
  a: handleBodyStartA,
...
function handleBodyEndHtml(self: any, token: Tag): ModeHandlerResult {
  if (self._in_scope("body")) return ["reprocess", InsertionMode.AFTER_BODY, token];
  return null;
}

function handleBodyEndP(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyEndLi(self: any, token: Tag): ModeHandlerResult {
...
function handleBodyEndDdDt(self: any, token: Tag): ModeHandlerResult {
  const name = token.name;
...
function handleBodyEndForm(self: any, token: Tag): ModeHandlerResult {
  if (self.form_element == null) {
    self._parse_error("unexpected-end-tag", token.name);
    return null;
  }
...
function handleBodyEndAppletLike(self: any, token: Tag): ModeHandlerResult {
  const name = token.name;
...
function handleBodyEndHeading(self: any, token: Tag): ModeHandlerResult {
  const name = token.name;
...
function handleBodyEndBlock(self: any, token: Tag): ModeHandlerResult {
  const name = token.name;
  if (!self._in_scope(name)) {
    self._parse_error("unexpected-end-tag", name);
    return null;
...
const BODY_END_HANDLERS: Record<string, ModeHandler> = {
...
function handleTagInBody(self: any, token: Tag): ModeHandlerResult {
  if (token.kind === Tag.START) {
...
function handleEofInBody(self: any, token: EOFToken): ModeHandlerResult {
...
function modeInBody(self: any, token: any): ModeHandlerResult {
...
function modeAfterBody(self: any, token: any): ModeHandlerResult {
...
function modeAfterAfterBody(self: any, token: any): ModeHandlerResult {
  if (token instanceof CommentToken) {
    if (self.fragment_context != null) {
...
function modeInTable(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    let data = token.data || "";
    if (data.includes("\x00")) {
...
function modeInTableText(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    let data = token.data;
    if (data.includes("\x0c")) {
...
function modeInCaption(self: any, token: any): ModeHandlerResult {
...
function modeInColumnGroup(self: any, token: any): ModeHandlerResult {
  const current = self.open_elements.length
    ? self.open_elements[self.open_elements.length - 1]
    : null;
  ...
  if (token instanceof Tag) {
    ...
    if (token.kind === Tag.START) {
      ...
      if (
        ...
        (self.fragment_context.tag_name || self.fragment_context.tagName || "").toLowerCase() ===
          "colgroup" &&
...
function modeInTableBody(self: any, token: any): ModeHandlerResult {
...
function modeInRow(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken || token instanceof CommentToken)
    return modeInTable(self, token);
...
function modeInCell(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    const previous = self.insert_from_table;
...
function modeInFrameset(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    const data = token.data || "";
    let whitespace = "";
...
function modeAfterFrameset(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    const data = token.data || "";
    let whitespace = "";
    for (const ch of data) {
...
function modeAfterAfterFrameset(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    if (isAllWhitespace(token.data)) {
...
function modeInSelect(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) {
    let data = token.data || "";
    if (data.includes("\x00")) {
...
function modeInTemplate(self: any, token: any): ModeHandlerResult {
  if (token instanceof CharacterToken) return modeInBody(self, token);
...
function modeFallbackToBody(self: any, token: any): ModeHandlerResult {
  self.mode = InsertionMode.IN_BODY;
  return ["reprocess", InsertionMode.IN_BODY, token];
}
...
export class TreeBuilder {
  ...
  quirks_mode: any;
  table_text_original_mode: any;
  template_modes: any;
  tokenizer: any;
  tokenizer_state_override: any;
  constructor(
    fragment_context: FragmentContext | null = null,
    iframe_srcdoc = false,
  ...
  _has_element_in_scope(
    ...
    for (let idx = this.open_elements.length - 1; idx >= 0; idx -= 1) {
      ...
      const matchesTarget =
        typeof target === "string" ? node.name === target : target.has(node.name);
      ...
      if (ns === "html" || ns == null) {
        ...
        (this._is_html_integration_point(node) || this._is_mathml_text_integration_point(node))
      ) {
        return false;
      }
    }
  ...
  _has_element_in_button_scope(target: string) {
    return this._has_element_in_scope(target, BUTTON_SCOPE_TERMINATORS);
  }

  _pop_until_inclusive(name: any) {
  ...
  _pop_until_any_inclusive(names: any) {
    while (this.open_elements.length) {
      const node = this.open_elements.pop();
  ...
  _generate_implied_end_tags(exclude: string | null = null) {
    while (this.open_elements.length) {
  ...
  processToken(token: any) {
    ...
    while (true) {
      ...
      const isHtmlNamespace =
        currentNode == null || currentNode.namespace == null || currentNode.namespace === "html";
  ...
  _append_comment_to_document(text: any) {
    this.document.append_child(new Node("#comment", { data: text, namespace: null }));
  }

  _append_comment(text: any, parent: any = null) {
    let target = parent ?? this._current_node_or_html();
    if (!target) return;
    if (isTemplateNode(target)) target = target.templateContent ?? target;
    if (!target) return;
    target.append_child(new Node("#comment", { data: text, namespace: null }));
  }

  _append_text(text: any) {
    if (!text) return;
    if (this.ignore_lf) {
      this.ignore_lf = false;
      if (text.startsWith("\n")) {
  ...
  _insert_element(tag: any, { push, namespace = "html" }: any = {}) {
  ...
  _create_element(name: any, namespace: any, attrs: any) {
  ...
  _add_missing_attributes(node: Node, attrs: AttributeMapInput) {
    ...
    for (const [name, value] of Object.entries(attrs)) {
      if (!Object.prototype.hasOwnProperty.call(existing, name)) existing[name] = value ?? null;
    }
    node.attrs = existing;
  }

  _remove_from_open_elements(node: any) {
  ...
  _find_active_formatting_index_by_node(node: any) {
    for (let index = this.active_formatting.length - 1; index >= 0; index -= 1) {
      const entry = this.active_formatting[index];
  ...
  _clone_attributes(attrs: AttributeMapInput): AttributeMap {
    return attrs ? { ...attrs } : {};
  }

  _attrs_signature(attrs: AttributeMapInput) {
    if (!attrs) return "";
  ...
  _find_active_formatting_duplicate(name: any, attrs: AttributeMapInput) {
  ...
  _has_active_formatting_entry(name: any) {
    for (let index = this.active_formatting.length - 1; index >= 0; index -= 1) {
      const entry = this.active_formatting[index];
      if (entry === FORMAT_MARKER) break;
  ...
  _remove_last_active_formatting_by_name(name: any) {
    for (let index = this.active_formatting.length - 1; index >= 0; index -= 1) {
      const entry = this.active_formatting[index];
      if (entry === FORMAT_MARKER) break;
      if (entry.name === name) {
  ...
  _remove_last_open_element_by_name(name: any) {
  ...
  _append_active_formatting_entry(name: any, attrs: AttributeMapInput, node: any) {
  ...
  _remove_formatting_entry(index: any) {
  ...
  _insert_node_at(parent: any, index: any, node: any) {
  ...
  _appropriate_insertion_location(override_target = null, { foster_parenting = false } = {}) {
    ...
    if (foster_parenting && TABLE_FOSTER_TARGETS.has(target.name)) {
      ...
      if (
        lastTemplate &&
        (lastTable == null ||
          this.open_elements.indexOf(lastTemplate) > this.open_elements.indexOf(lastTable))
      ) {
  ...
  _has_in_scope(name: any) {
    return this._has_element_in_scope(name, DEFAULT_SCOPE_TERMINATORS);
  }

  _has_in_list_item_scope(name: any) {
    return this._has_element_in_scope(name, LIST_ITEM_SCOPE_TERMINATORS);
  }

  _has_in_definition_scope(name: any) {
    return this._has_element_in_scope(name, DEFINITION_SCOPE_TERMINATORS);
  }

  _has_any_in_scope(names: any) {
    const terminators = DEFAULT_SCOPE_TERMINATORS;
    for (let idx = this.open_elements.length - 1; idx >= 0; idx -= 1) {
      const node = this.open_elements[idx];
      if (names.has(node.name)) return true;
      if ((node.namespace == null || node.namespace === "html") && terminators.has(node.name))
        return false;
    }
    return false;
  }

  _populate_selectedcontent(root: any) {
    const selects: any = [];
  ...
  _find_elements(node: any, name: any, result: any) {
    if (!node) return;
  ...
  _find_element(node: Node | null, name: string): Node | null {
    if (!node) return null;
    if (node.name === name) return node;

    for (const child of node.children || []) {
      const found: Node | null = this._find_element(child, name);
  ...
  _clone_children(source: any, target: any) {
    for (const child of source.children || []) {
      target.append_child(child.cloneNode(true));
    }
  }

  _should_foster_parenting(target: any, { forTag = null, isText = false } = {}) {
  ...
  _prepare_foreign_attributes(namespace: string | null, attrs: AttributeMapInput): AttributeMap {
    if (!attrs) return {};
  ...
  _is_html_integration_point(node: any) {
  ...
  _is_mathml_text_integration_point(node: any) {
  ...
  _should_use_foreign_content(token: any) {
    const current = this.open_elements[this.open_elements.length - 1];
