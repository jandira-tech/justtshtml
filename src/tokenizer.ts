...
function isAsciiAlpha(c: any) {
  const code = c.charCodeAt(0);
  return (code >= 0x41 && code <= 0x5a) || (code >= 0x61 && code <= 0x7a);
}

function asciiLower(c: any) {
  const code = c.charCodeAt(0);
  if (code >= 0x41 && code <= 0x5a) return String.fromCharCode(code + 0x20);
  return c;
}

function coerceTextForXML(text: any) {
  if (!text) return text;
...
function coerceCommentForXML(text: any) {
  if (!text) return text;
...
export class Tokenizer {
  ...
  collectErrors: any;
  currentAttrName: any;
  currentAttrValue: any;
  currentAttrValueHasAmp: any;
  currentChar: any;
  currentComment: any;
  currentDoctypeForceQuirks: any;
  currentDoctypeName: any;
  currentDoctypePublic: any;
  currentDoctypeSystem: any;
  currentTagAttrs: any;
  currentTagKind: any;
  currentTagName: any;
  ...
  constructor(sink: any, opts = new TokenizerOpts(), { collectErrors = false } = {}) {
    this.sink = sink;
    this.opts = opts;
  ...
  _emitToken(token: any) {
    this.sink.processToken(token);
  }

  _emitError(code: any) {
    if (!this.collectErrors) return;
    this.errors.push(new ParseError(code));
  }
  ...
  _consumeCaseInsensitive(literal: any) {
    const end = this.pos + literal.length;
    if (end > this.length) return false;
  ...
  _stateAfterDoctypeName() {
    // eslint-disable-next-line no-constant-condition
    while (true) {
      const c = this.pos < this.length ? this.buffer[this.pos] : null;
      if (c == null) {
        this._emitError("eof-in-doctype");
        this.currentDoctypeForceQuirks = true;
        this._emitDoctype();
