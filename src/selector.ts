...
class SimpleSelector {
  ...
  constructor(selectorType: any, { name = null, operator = null, value = null, arg = null } = {}) {
...
class SelectorParser {
  ...
  parse() {
    const selectors = [];
    const firstSelector = this._parseComplexSelector();
    if (!firstSelector) throw new SelectorError("Empty selector");
    selectors.push(firstSelector);

    while (this._peek().type === TokenType.COMMA) {
      this._advance();
      const selector = this._parseComplexSelector();
      if (!selector) throw new SelectorError("Empty selector");
  ...
  _parsePseudoSelector() {
    ...
    if (this._peek().type === TokenType.PAREN_OPEN) {
      this._advance();
      let arg = null;
      if (this._peek().type === TokenType.PAREN_CLOSE)
        throw new SelectorError("Empty pseudo-class argument");
...
class SelectorMatcher {
  ...
  _matchesComplex(node: any, selector: any) {
  ...
  _matchesCompound(node: any, compound: any) {
  ...
  _matchesAttribute(node: any, selector: any) {
  ...
  _matchesPseudo(node: any, selector: any): boolean {
    ...
    if (name === "root") {
      ...
      return (
        parent != null && (parent.name === "#document" || parent.name === "#document-fragment")
      );
    }
  ...
  _getElementChildren(parent: any) {
    if (!parent || !Array.isArray(parent.children) || !parent.children.length) return [];
    return parent.children.filter((c: any) => isElementNode(c));
  }

  _getPreviousSibling(node: any) {
    const parent = node.parent;
  ...
  _matchesNth(index: any, a: any, b: any) {
  ...
  _matchesNthChild(node: any, arg: any) {
    const parent = node.parent;
    if (!parent) return false;
  ...
  _matchesNthOfType(node: any, arg: any) {
    const parent = node.parent;
    if (!parent) return false;
...
function parseSelector(selectorString: any) {
...
function queryDescendants(node: any, selector: any, results: any) {
...
export function query(root: any, selectorString: any) {
...
export function matches(node: any, selectorString: any) {
  const selector = parseSelector(selectorString);
